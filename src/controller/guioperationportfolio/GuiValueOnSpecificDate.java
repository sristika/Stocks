...
/**
 * Class represents the value of the current portfolio on a specific date through a GUI.
 */
public class GuiValueOnSpecificDate implements GuiOperations {
  ...
  public String executeOperation(String data, InvestmentStrategyImpl model) {
    try {
      ...
      String result = model.valueOfPortfolioOnSpecificDate(lines[0], stockObject, df.format(date));
      if (result.equals("")) {
        return "false";
      } else {
        return result;
