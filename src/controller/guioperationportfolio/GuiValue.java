...
/**
 * Class represents the value of the current portfolio through a GUI.
 */
public class GuiValue implements GuiOperations {
  ...
  public String executeOperation(String data, InvestmentStrategyImpl model) {
    try {
      ...
      String result = model.valueOfPortfolio(lines[0], stockObject, df.format(date));
      if (result.equals("")) {
        return "false";
      } else {
        return result;
